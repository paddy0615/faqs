...
public interface Folder_library_relationDao extends JpaRepository<Folder_library_relation,Long> {
     ...
     */
    @Query(value ="SELECT r.flr_parenid,l.fl_id,l.fl_title,DATE_FORMAT(r.flr_createdate, '%Y-%m-%d %H:%i') AS 'createdate',r.flr_id FROM folder_library_relation r,faqs_librabry l" +
    ...
    @Modifying
    @Query(value =" DELETE FROM folder_library_relation WHERE flr_parenid = :key",nativeQuery = true)
